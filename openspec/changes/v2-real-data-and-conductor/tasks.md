...
### T1 (Beads: clawde-27w.1): Create client state loader
...
### T2 (Beads: clawde-27w.2): Boot store from real data (no mocks by default)
...
### T3 (Beads: clawde-27w.3): Wire hydration into app lifecycle
- Call `hydrateFromApi()` on app mount
- Ensure initial render shows a clear loading/empty state
- **Deps:** T2

### T4 (Beads: clawde-27w.4): Verify empty states across screens
...
### T5 (Beads: clawde-27w.5): Wire SSE updates to store hydration
...
### T6 (Beads: clawde-27w.6): Conductor state model
...
### T7 (Beads: clawde-27w.7): Implement handoff/queue/next/interrupt commands
- Add commands (accepted in both `/` and `clawde` forms):
  - `handoff`, `queue`, `next`, `interrupt`
- Enforce: one in-flight execution at a time
- **Deps:** T6

### T8 (Beads: clawde-27w.8): Confirmation tokens (typed confirm/cancel)
...
### T9 (Beads: clawde-27w.9): Update demo capture to show marquee v2 loop
...
### T10 (Beads: clawde-27w.10): README “new features” highlight block
...
- Epic: `clawde-27w`
- Tasks: `clawde-27w.1` … `clawde-27w.10` (IDs stamped above)
- Dependencies wired to match the **Deps:** lines

Backlog reconciliation:
- `clawde-qpj` (v1-T24 HQ demo assets redo): **closed** (done; assets shipped in v1)
- `clawde-vu8` (v1-T23 Conductor / floor control): **closed** (superseded by v2; see `clawde-27w.6`)
